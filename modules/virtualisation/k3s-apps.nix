...
let
  ...
  stackType = lib.types.submodule ({ name, config, ... }: {
    options = {
          ...
          options = {
            ...
            service = lib.mkOption {
              type = lib.types.submodule {
                options = {
                  type = lib.mkOption {
                    type = lib.types.enum [ "ClusterIP" "NodePort" "LoadBalancer" ];
  ...
  mkAppManifests = stackName: stackCfg: appName: appCfg:
    let
      baseManifests = lib.optionalAttrs (appCfg.env != {}) {
        "${stackName}-${appName}-configmap" = mkConfigMap stackCfg.namespace appName appCfg.env;
      } // lib.optionalAttrs (appCfg.storage != null) {
        "${stackName}-${appName}-pv" = mkPersistentVolume stackCfg.storageClass appName appCfg.storage.path appCfg.storage.size;
        
        "${stackName}-${appName}-pvc" = mkPersistentVolumeClaim stackCfg.namespace stackCfg.storageClass appName appCfg.storage.size;
      } // {
        "${stackName}-${appName}-deployment" = mkDeployment stackCfg.namespace appName (
          if appCfg.useSharedStorage && stackCfg.sharedStorage != null
          then appCfg // {
  ...
  generateStackManifests = stackName: stackCfg:
    let
      baseManifests = {
        "${stackName}-namespace" = {
          apiVersion = "v1";
        ...
        "${stackName}-shared-pv" = mkPersistentVolume 
          stackCfg.storageClass 
          stackCfg.sharedStorage.pvName
          stackCfg.sharedStorage.path 
          stackCfg.sharedStorage.size;
        
        "${stackName}-shared-pvc" = mkPersistentVolumeClaim 
          stackCfg.namespace 
          stackCfg.storageClass 
          stackCfg.sharedStorage.pvName
          stackCfg.sharedStorage.size;
      ...
      appManifests = lib.foldl' (acc: appName:
        let appCfg = stackCfg.apps.${appName};
        in if appCfg.enable
           then acc // mkAppManifests stackName stackCfg appName appCfg
           else acc
  ...
  k3sStacks = lib.filterAttrs (name: cfg: 
    lib.hasPrefix "k3s-" name && (cfg.enable or false)
  ) allStacks;

  # Generate all manifests
  allManifests = lib.foldl' (acc: stackName:
    acc // generateStackManifests stackName k3sStacks.${stackName}
