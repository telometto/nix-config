...
/*
  ...
  {
    # Enable option
    options.telometto.k3s.download-management = {
    ...
    config.services.k3s.manifests =
      lib.mkIf (config.telometto.k3s.download-management.enable or false)
        {
          # Namespace
    ...
    config.telometto.services.traefik.services =
      lib.mkIf (config.telometto.k3s.download-management.enable or false)
        {
          firefox = {
            backendUrl = "http://192.168.2.100:5859/";
            pathPrefix = "/firefox";
            stripPrefix = true;
            customHeaders = {
