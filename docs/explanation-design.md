...
The HM loader's exclusion of `overrides/role/`, `overrides/host/`, and `overrides/user/` is intentional. `home-users.nix` opts into only the active role, host, user-wide, and user@host files. Host overrides remain isolated to one machine, while `home/overrides/user/<username>.nix` intentionally follows that user across machines. The host-loader receiving `hostname` via `specialArgs` means each host's system config is completely isolated in its own directory tree — renaming a directory is all you need to move a host configuration.
...
Home Manager integrates at the NixOS level for each enabled user, merging a shared template, optional extra modules, and role, host-wide, user-wide, and user@host overrides. Desktop flavors auto-signal HM to enable relevant pieces (`hm.desktop.<flavor>.enable = true`).

Config is assembled by `modules/core/home-users.nix` in a specific order. `hm-loader.nix` is imported first, making the shared `home/*.nix` module set available for every user. Next come `sys.home.extraModules`, `sys.home.template.imports`, the optional role and host overrides, the optional user-wide override (`home/overrides/user/<username>.nix`), the optional user@host override (`home/overrides/user/<username>-<hostname>.nix`), and finally any per-user `extraModules` plus `sys.home.users.<username>.extraConfig.imports`. Import order does not itself resolve conflicting Nix option definitions, so shared defaults use `lib.mkDefault` or an explicit `lib.mkOverride` priority. Separately, `sys.home.users.<username>.extraConfig` attribute values are merged before `autoDesktopConfig`; because `autoDesktopConfig` uses `lib.mkDefault`, explicit user settings still override the desktop-derived defaults. `lib.mkForce` in any override file always wins regardless of layer. `home/base.nix` provides many of the defaults — most programs are enabled by default — so opting out is often more common than opting in.
