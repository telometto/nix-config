...
Config is assembled by `modules/core/home-users.nix` in a specific order. `hm-loader.nix` is imported first, making the shared `home/*.nix` module set available for every user. Next come `sys.home.extraModules`, then `sys.home.template.imports`, then the optional host override (`home/overrides/host/<hostname>.nix`), then the optional user@host override (`home/overrides/user/<username>-<hostname>.nix`), and finally any per-user `extraModules` plus `sys.home.users.<username>.extraConfig.imports`. Separately, `sys.home.users.<username>.extraConfig` attribute values are merged before `autoDesktopConfig`; because `autoDesktopConfig` uses `lib.mkDefault`, explicit user settings still override the desktop-derived defaults. `lib.mkForce` in any override file always wins regardless of layer. `home/base.nix` provides many of the defaults — most programs are enabled by default — so opting out is often more common than opting in.
