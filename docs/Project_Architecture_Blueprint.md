...
flowchart TD
    ...
    OUT["nixosConfigurations.*-vm\n(26 entries merged into flake outputs)"]
...
flowchart TD
    ...
    ADG["adguard-vm\n10.100.0.10\nDNS sinkhole"]
    WG["wireguard-vm\n10.100.0.11\nVPN gateway"]
    REST["Other VMs\nselected 10.100.0.12–71 addresses + 10.100.0.80\ndefault gateway = 10.100.0.1"]
    PIDBR["pocket-id-br0\n10.100.1.0/30\ndedicated L2 + host L3 filter"]
    PID["pocket-id-vm\n10.100.1.2"]
...
when it originates from Blizzard's dedicated-bridge address.
