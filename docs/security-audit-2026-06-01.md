...
**Status:** Superseded by the Matrix baseline hardening change; historical evidence\
**Confidence:** Medium

The earlier Matrix middleware disabled CSP and allowed wildcard CORS with
authorization headers across the whole Matrix hostname. That behavior has been
removed. Current CORS exceptions are limited to the explicit Matrix discovery
responses generated by [`vms/matrix-synapse.nix`](../vms/matrix-synapse.nix),
while the remaining Matrix middleware is still a protocol-specific exception
that must not be reused for unrelated services.

Evidence:

- Historical evidence: `hosts/blizzard/security/traefik.nix` previously set
  `Access-Control-Allow-Origin = "*"` for the Matrix middleware.
- Current evidence: [`hosts/blizzard/security/traefik.nix`](../hosts/blizzard/security/traefik.nix)
  no longer adds that wildcard response header.
- Current discovery-only CORS: [`vms/matrix-synapse.nix`](../vms/matrix-synapse.nix)
  adds the header only to the three explicit `/.well-known` responses.

Impact:

- Future maintainers may copy the remaining Matrix-specific headers to
  non-Matrix services.
- The explicit discovery CORS exceptions should remain narrow and should not be
  expanded to token-bearing Matrix API routes without a compatibility test.

Remediation:

1. Keep the Matrix middleware documented as Matrix-only and do not reuse it for
   unrelated services.
1. Keep CORS on explicit discovery responses rather than the whole hostname.
1. Revisit the middleware name if a future refactor makes its protocol scope
   ambiguous.
