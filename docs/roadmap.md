...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| R-02 | Restore forensic HTTP detection and client attribution, with durable security-log retention | Observability / Security | Now | In progress | Off-host retention path | HTTP source fixes and regression coverage are implemented; validate parsing and attribution after approved deployment, then implement persistent/off-host retention and auditd execution telemetry for R-13 | [crowdsec.nix](../hosts/blizzard/security/crowdsec.nix), [traefik.nix](../hosts/blizzard/security/traefik.nix), [HTTP regression check](../tests/crowdsec-http.nix) | [HTTP detection contract and acceptance](crowdsec-http-detection.md), [Blizzard handoff](2026-08-14-blizzard-paranoid-nixos-handoff.md) |
| R-03 | Harden the Matrix baseline before adding OIDC | Matrix / Security | Now | In progress | R-01 | Complete the remaining live acceptance matrix; the four public availability contracts now have read-only smoke evidence, and the seven-day clean observation gate starts only after the full matrix passes | [matrix-synapse.nix](../vms/matrix-synapse.nix), [Matrix services](../modules/services/), [matrix-baseline.nix](../tests/matrix-baseline.nix) | [Matrix hardening plan](matrix-hardening-plan.md), [security sequence](security-roadmap-implementation-order.md) |
...
| R-15 | Add `mautrix-whatsapp` as a durable, narrowly scoped Matrix appservice without a public bridge endpoint | Matrix / Integration | Next | In progress | R-03 Matrix acceptance and observation; R-04 backup evidence before activation | Declarative pre-login wiring is present but remains disabled by default; after the Matrix and backup gates, provision SOPS values and validate registration, QR login, reconnect, and restore behavior | [Matrix–WhatsApp bridge design](matrix-whatsapp-bridge.md), [matrix-synapse.nix](../vms/matrix-synapse.nix), [Matrix MicroVM](../hosts/blizzard/virtualisation/microvms.nix), [MicroVM reference](../vms/README.md) | [Matrix–WhatsApp bridge design](matrix-whatsapp-bridge.md), [mautrix/whatsapp](https://github.com/mautrix/whatsapp) |
...
| R-16 | Separate guest MicroVM configurations into an independently updatable flake | Platform / MicroVM | Later | Candidate | Stable host/VM contract and a self-contained external flake | Create a separate GitHub flake exporting matching `nixosConfigurations.<name>-vm` outputs; consume it as both the host-side `flake` and `vmConfig.updateFlake`, then validate `microvm -Ru <name>-vm` for guest-only updates | [VM configurations](../vms/), [MicroVM host adapter](../hosts/blizzard/virtualisation/microvms.nix), [MicroVM base](../modules/virtualisation/microvm-base.nix), [VM registry](../vms/vm-registry.nix) | Keep host orchestration, publication, forwarding, and network policy in `nix-config`. Share or export registry identity, and make the external flake provide its own `nix-secrets`, `VARS`, `consts`, and dependencies. Host wiring and shared-contract changes still require a Blizzard rebuild; `updateFlake` alone must not become a second source of truth. |
