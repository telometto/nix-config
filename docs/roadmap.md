...
| R-15 | Add `mautrix-whatsapp` as a durable, narrowly scoped Matrix appservice without a public bridge endpoint | Matrix / Integration | Next | Shaping | R-03 Matrix acceptance and observation; R-04 backup evidence before activation | After the Matrix gate, evaluate the locked Nixpkgs module, add the bridge inside `matrix-synapse-vm` with separate state/database storage, and validate registration, QR login, reconnect, and restore behavior | [Matrix–WhatsApp bridge design](matrix-whatsapp-bridge.md), [matrix-synapse.nix](../vms/matrix-synapse.nix), [Matrix MicroVM](../hosts/blizzard/virtualisation/microvms.nix), [MicroVM reference](../vms/README.md) | [Matrix–WhatsApp bridge design](matrix-whatsapp-bridge.md), [mautrix/whatsapp](https://github.com/mautrix/whatsapp) |
